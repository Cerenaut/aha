...
from classifier_component.harness import Harness  # pylint: disable=C0413
...
class Workflow(object):
  ...
  def _setup_dataset(self):
    ...
    with tf.variable_scope('dataset'):
      ...
      if self._opts['train_classes']:
        train_classes = list(map(int, self._opts['train_classes']))
        train_dataset = train_dataset.filter(lambda x, y: tf_label_filter(x, y, train_classes))
      ...
      if self._opts['test_classes']:
        test_classes = list(map(int, self._opts['test_classes']))
        eval_train_dataset = eval_train_dataset.filter(lambda x, y: tf_label_filter(x, y, test_classes))
      ...
      if self._opts['test_classes']:
        test_classes = list(map(int, self._opts['test_classes']))
        eval_test_dataset = eval_test_dataset.filter(lambda x, y: tf_label_filter(x, y, test_classes))
  ...
  def _on_after_training_batch(self, batch, training_step, training_epoch):
    ...
    logger_utils.log_metrics({'mse': loss})
