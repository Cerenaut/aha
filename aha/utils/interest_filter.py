...
class InterestFilter:
  ...
  def summarize_level(self):
    return self._hparams.summarize_level
  ...
  def add_summaries(self, summaries):
    ...
    if show_all_separately:
      ...
      if show_encs_reduced:
        ...
        mask_image_reduced = tf.reduce_max(mask_image, axis=3, keepdims=True)    # across filters

        pos_image = self._image_dic['positional_encodings']
        pos_image_reduced = tf.reduce_max(pos_image, axis=3, keepdims=True)    # across filters
  ...
  def _depthwise_conv_with_2d_filter(self, in_tensor, filter2d):
    """ Depthwise means it convs each filter in z (of in_tensor) independently"""
